...
$(function() {
    ...
    if (!!scheds) {
        Object.keys(scheds).map(k => {
            var row = scheds[k]
            events.push({ id: row.id, title: row.title,worker_id: row.worker_id, start: row.start_datetime, end: row.end_datetime });
        });
    }
    ...
    calendar = new Calendar(document.getElementById('calendar'), {
        ...
        eventClick: function(info) {
            ...
            if (!!scheds[id]) {
                ...
                details.find('#worker_id').text(scheds[id].title);
    ...
    $('#edit').click(function() {
        ...
        if (!!scheds[id]) {
            ...
            form.find('[name="worker_id"]').val(id);
